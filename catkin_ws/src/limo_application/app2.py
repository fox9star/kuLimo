...
<body>
    ...
    <button onclick="sendOneCommand()">One</button>
    <button onclick="sendTwoCommand()">Two</button>
    <script>
        ...
        function sendOneCommand() {
            ws.send("one");
        }
        function sendTwoCommand() {
            ws.send("two");
        }
    </script>
</body>
...
def move_limo():
    ...
    while count < 100 and not rospy.is_shutdown():
    ...
    try:
        while True:
            ...
            if message == "go":
                ...
                pub.publish(message)  # "one"이나 "two"도 여기서 publish됨
